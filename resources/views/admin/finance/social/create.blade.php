@extends('layouts.app')
@push('title')
<title>Tambah Dana Bantuan | {{ config('setting.name') }}</title>
@endpush
@push('style')
<link href="{{ asset('assets/plugins/litepicker/litepicker.css') }}" rel="stylesheet">
@endpush
@section('content')
<h1 class="app-page-title">Tambah Dana Bantuan</h1>
<hr class="mb-4">
<div class="app-card app-card-settings shadow-sm p-4">
    <div class="app-card-body">
        ...
        @endif
        <form class="settings-form" method="POST" action="{{ route('admin.financesocial.store') }}">
            @csrf
            @include('admin.finance.social.form')
            <button type="submit" class="btn app-btn-primary w-100">Tambah Dana Bantuan</button>
        </form>
    </div>
</div>
@push('script')
